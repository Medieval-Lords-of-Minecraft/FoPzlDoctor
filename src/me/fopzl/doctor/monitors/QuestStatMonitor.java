...
public class QuestStatMonitor extends Monitor {
	...
	private enum LevelGroup {
		...
		T3MID("40-49"),
		T3HIGH("50-59"),
		MAX("60"); // will need to update when T4 comes out in 10 years
		...
		public static LevelGroup fromLevel(int lvl) {
			if(lvl < 10) return T2LOW;
			if(lvl < 20) return T2MID;
			if(lvl < 30) return T2HIGH;
			if(lvl < 40) return T3LOW;
			if(lvl < 50) return T3MID;
			if(lvl < 60) return T3HIGH;
			return MAX;
		}
